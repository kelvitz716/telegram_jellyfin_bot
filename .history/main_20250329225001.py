...
class MediaManagerApp:
    ...
    async def setup_file_watcher(self) -> FileWatcher:
        ...
        # Modify the event handler to use an async callback
        def async_callback(filepath):
            # Create a task to run the async method
            asyncio.create_task(self.process_new_file(filepath))
        
        watcher.create_event_handler(
            on_created=async_callback,
