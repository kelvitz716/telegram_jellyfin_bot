...
class ProgressTracker:
    ...
    async def send_progress(self) -> None:
        ...
        try:
            ...
            # Progress bar with improved Unicode characters (full width)
            bar_length = 20
            filled_length = int(bar_length * percent / 100)
            # Using better block elements for more aesthetic progress bar
            ...
            stats = (
                f"Speed: {speed_text}\n"
                f"ETA: {eta_text}\n"
                f"Size: {self._format_size(self.downloaded)} of {self._format_size(self.total_size)}"
            )
