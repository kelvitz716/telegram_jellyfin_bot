...
from file_processing.media_identifier import MediaFilenameParser
...
class MediaManagerApp:
    ...
    async def process_new_file(self, filepath: str):
        """
        Process newly detected media file with comprehensive error handling.
        ...
        try:
            ...
            if not os.path.isfile(filepath):
                self.logger.warning(f"Not a valid file: {filepath}")
                return

            # Log detection
            self.logger.info(f"New file detected: {filepath}")

            # Use run_in_executor for potentially blocking file operations
            categorized_path = await asyncio.get_event_loop().run_in_executor(
                self.executor, 
    ...
    def _categorize_file(self, filepath: str) -> str:
        """
        Categorize file based on configuration and media type.
        ...
        """
        # Use MediaFilenameParser to identify media type
        try:
            # Parse filename to get media metadata
            metadata = MediaFilenameParser.parse_filename(os.path.basename(filepath))
            
            # Get paths from configuration
            paths = self.config_manager.config['paths']
            
            # Determine destination directory based on media type
            if metadata['type'] == 'tv_show':
                base_dir = paths.get('tv_shows_dir', paths.get('download_dir'))
                show_name = metadata.get('show_name', 'Unknown Show')
                season = metadata.get('season', 0)
                
                # Create show and season directories
                dest_dir = os.path.join(base_dir, show_name, f'Season {season}')
                os.makedirs(dest_dir, exist_ok=True)
                
                # Generate new filename
                new_filename = (
                    f"{show_name} - S{season:02d}E{metadata.get('episode', 0):02d}"
                    f"{os.path.splitext(filepath)[1]}"
                )
                dest_path = os.path.join(dest_dir, new_filename)
            
            elif metadata['type'] == 'movie':
                base_dir = paths.get('movies_dir', paths.get('download_dir'))
                title = metadata.get('title', 'Unknown Movie')
                year = metadata.get('year', 'Unknown')
                
                # Create year directory
                dest_dir = os.path.join(base_dir, str(year))
                os.makedirs(dest_dir, exist_ok=True)
                
                # Generate new filename
                new_filename = f"{title} ({year}){os.path.splitext(filepath)[1]}"
                dest_path = os.path.join(dest_dir, new_filename)
            
            else:
                # Unmatched media
                base_dir = paths.get('unmatched_dir', paths.get('download_dir'))
                dest_path = os.path.join(base_dir, os.path.basename(filepath))
            ...
            # If categorization fails, move to unmatched directory
            unmatched_dir = self.config_manager.config['paths'].get(
                'unmatched_dir', 
                self.config_manager.config['paths'].get('download_dir')
            )
            os.makedirs(unmatched_dir, exist_ok=True)
            dest_path = os.path.join(unmatched_dir, os.path.basename(filepath))
            os.rename(filepath, dest_path)
            
            self.logger.error(f"Categorization error for {filepath}: {e}")
            return dest_path

    async def setup_file_watcher(self):
        """
        Configure and start file system watcher with thread-safe queue processing.

        Returns:
            Configured file watcher and run coroutine
        """
        ...
        def sync_file_handler(event):
            """
            Synchronous wrapper to safely queue files from watchdog thread
            """
            try:
                # Check if this is a file event (added safety check)
                if hasattr(event, 'is_directory') and not event.is_directory:
                    filepath = event.src_path
        
                    # Use run_coroutine_threadsafe to add to queue from any thread
                    if self.background_loop:
                        future = asyncio.run_coroutine_threadsafe(
                            file_queue.put(filepath), 
                            self.background_loop
                        )
                        future.result(timeout=5)  # Wait for queuing to complete
            except Exception as e:
                print(f"Error queuing file: {e}")

        # Modify the watcher to use the sync handler
        watcher = FileWatcher(self.download_dir)
        watcher.create_event_handler(
            on_created=sync_file_handler,
            extensions=['.mkv', '.mp4', '.avi']
        )

        watcher.start()

        # Queue processing coroutine
        async def process_file_queue():
            ...
            while True:
                try:
                    ...
                    self.logger.error(f"Unexpected error in queue processing: {e}")

        return watcher, process_file_queue
    
    async def start(self):
        """
        Start the media manager application with improved concurrency.
        """
        self.logger.info("Starting Media Manager Application")

        try:
            # Setup event loop for background tasks
            self.background_loop = asyncio.get_running_loop()
            ...
            if self.telegram_client:
                ...
                # Setup file watcher with queue processing
                file_watcher, queue_processor = await self.setup_file_watcher()
    ...
    Args:
        ...
        download_path: Path to save downloaded file (will be adjusted)
    ...
    download_dir = self.config_manager.get('paths', 'download_dir') or os.path.expanduser('~/Downloads')
    
    # Ensure download directory exists
    os.makedirs(download_dir, exist_ok=True)
    
    # Generate a unique filename based on the original filename
    original_filename = event.file.name or f"telegram_download_{int(time.time())}"
                ...
                async with download_semaphore, rate_limit_semaphore:
                    ...
                    with tqdm.tqdm(
                        ...
                        async def progress_callback(current, total):
                            ...
                            # Throttle status message updates
                            current_time = time.time()
                            if current_time - last_update_time >= update_interval or current >= total:
                                try:
                                    # Safely get formatted values
                                    n_fmt = self._format_size(current)
                                    total_fmt = self._format_size(total)
                                    rate_fmt = self._format_size(
                                        current / (current_time - progress_bar.start_t) if current_time > progress_bar.start_t else 0
                                    ) + '/s'
                                    
                                    # Calculate progress percentage
                                    progress_percent = (current / total) * 100
                                    
                                    # Create more compact progress bar
                                    progress_bar_str = self._create_mobile_progress_bar(progress_percent)
                                    
                                    # Prepare status message with cleaner, more compact layout
                                    status_text = (
                                        f"📥 *Downloading:*\n"
                                        f"`{original_filename}`\n\n"
                                        f"{progress_bar_str}\n"
                                        f"*Progress:* {progress_percent:.1f}%\n"
                                        f"*Downloaded:* {n_fmt} / {total_fmt}\n"
                                        f"*Speed:* {rate_fmt}"
                                    )
    ...
    def _create_mobile_progress_bar(
    self, 
    progress_percent: float, 
    width: int = 20
    ) -> str:
        """
        Create a mobile-friendly progress bar.
        ...
        # Use color gradients based on progress
        if progress_percent < 25:
            color = '🔴'  # Red
        elif progress_percent < 50:
            color = '🟠'  # Orange
        ...
        return f"{color} [{bar}] {progress_percent:.1f}%"
...
def main():
    ...
    try:
        ...
        if args.command == 'start':
            ...
            asyncio.run(MediaManagerApp(config_manager).start())
