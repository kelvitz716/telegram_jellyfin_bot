...
class ProgressTracker:
    ...
    async def send_progress(self) -> None:
        ...
        try:
            ...
            # Progress bar with improved Unicode characters
            bar_length = 20
            filled_length = int(bar_length * percent / 100)
            # Using block elements for a more aesthetic progress bar
            bar = '━' * filled_length + '░' * (bar_length - filled_length)
            
            # Format batch information if applicable
            batch_info = ""
            if self.total_files > 1:
                batch_info = f"{self.position+1}/{self.total_files}: "
            ...
            message = (
                ...
                f"{bar} {percent:.1f}%\n"
                f"{self._format_size(self.downloaded)} of {self._format_size(self.total_size)} at {speed_text}\n"
                f"⏱️ ETA: {eta_text} | Elapsed: {elapsed_text}"
            )
    ...
    async def _download_file(self, task: DownloadTask) -> None:
        ...
        try:
            ...
            self.stats_manager.update(
                ...
                total_bytes=self.download_stats.total_bytes + (file_size if success else 0),
    ...
    def get_stats(self) -> str:
        ...
        if successful > 0 and total_bytes > 0 and uptime > 0:
            ...
            if speed_bytes_per_sec < 1024:
                avg_speed = f"{speed_bytes_per_sec:.1f} B/s"
            elif speed_bytes_per_sec < 1024 * 1024:
                avg_speed = f"{speed_bytes_per_sec/1024:.1f} KB/s"
            else:
                avg_speed = f"{speed_bytes_per_sec/(1024*1024):.1f} MB/s"

            # Calculate average time per download
            if successful > 0:
                avg_time_secs = uptime / successful
                if avg_time_secs < 60:
                    avg_time = f"{avg_time_secs:.0f}s"
                elif avg_time_secs < 3600:
                    minutes = int(avg_time_secs // 60)
                    seconds = int(avg_time_secs % 60)
                    avg_time = f"{minutes}m {seconds}s"
                else:
                    hours = int(avg_time_secs // 3600)
                    minutes = int((avg_time_secs % 3600) // 60)
                    avg_time = f"{hours}h {minutes}m"

        # Find peak concurrent downloads
        peak_concurrent = self.max_concurrent_downloads  # Default to max allowed
...
class TelegramDownloader:
    ...
    async def _shutdown(self) -> None:
        ...
        try:
            ...
            self.download_manager.stats_manager.save_stats()
