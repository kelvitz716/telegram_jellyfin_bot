...
class MediaManagerApp:
    ...
    async def _download_media_with_tqdm(self, event, download_path, status_message):
        """
        Download media with tqdm progress and rate limiting.
        ...
        """
        Download with built-in rate limiting and retry mechanism.
        """
        max_retries = 5
        for attempt in range(max_retries):
            ...
            except NetworkMigrateError as migrate_error:
                # Handle network migration errors
                self.logger.warning(f"Network migrate error: {migrate_error}")
                # You might want to update the DC configuration here
                await asyncio.sleep(1)
            
            except RPCError as rpc_error:
                # Catch any other RPC errors
                self.logger.error(f"RPC error during download: {rpc_error}")
