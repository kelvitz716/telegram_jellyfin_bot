    ...
    async def process_media_group(self, media_group_id: str) -> None:
        ...
        try:
            ...
            for file_data in batch_tracker.files:
                if (file_data['file_id'] not in seen_file_ids and 
                    file_data['file_name'] not in seen_file_names):
                    seen_file_ids.add(file_data['file_id'])
                    seen_file_names.add(file_data['file_name'])
        ...
        async def handle_media(message):
            ...
            if not media_group_id:
                ...
                if hasattr(file_info, 'file_name') and file_info.file_name:
                    ...
                    # Try to extract from caption or message text if available
                    if hasattr(message, 'caption') and message.caption:
                        # Look for filename patterns in caption
                        import re
                        file_pattern = re.search(r'([\w\.-]+\.(mp4|mkv|avi|mov|flv|wmv))', message.caption)
                        if file_pattern:
                            file_name = file_pattern.group(1)
                        else:
                            file_name = f"file_{file_id}"
                    elif hasattr(message, 'text') and message.text:
                        # Try to extract from message text
                        import re
                        file_pattern = re.search(r'([\w\.-]+\.(mp4|mkv|avi|mov|flv|wmv))', message.text)
                        if file_pattern:
                            file_name = file_pattern.group(1)
                        else:
                            file_name = f"file_{file_id}"
                ...
                if media_group_id in self.download_manager.batch_trackers:
                    ...
                    existing_file_names = [f['file_name'] for f in batch_tracker.files]
                    if file_id in existing_file_ids or file_name in existing_file_names:
