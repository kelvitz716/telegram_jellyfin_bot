        ...
        for attempt in range(max_retries):
            try:
                ...
                if not await self._check_telethon_connection():
                    logger.error("Could not establish Telethon connection")
                ...
                if hasattr(message, 'chat') and hasattr(message, 'message_id'):
                    try:
                        ...
                        if telethon_message:
                            ...
                            logger.error("Could not retrieve original message via Telethon")
                            if attempt < max_retries - 1:
                                await asyncio.sleep(retry_delay)
                                continue
                            return False
                ...
                logger.error(f"Telethon download error (attempt {attempt + 1}/{max_retries}): {str(e)}")
                if attempt < max_retries - 1:
                    await asyncio.sleep(retry_delay)
                    continue
                return False
