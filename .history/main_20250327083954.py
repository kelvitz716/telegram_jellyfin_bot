    ...
    async def handle_media_download(self, event):
        ...
        try:
            ...
            async def progress_callback(current, total):
                """
                Provide download progress updates.
                ...
                if (current % (total // 10) == 0) or (time.time() -     download_start_time > 5):
                    progress_percent = (current / total) * 100
                    try:
                        await status_message.edit(f"⬇️ Downloading: {progress_percent:.  1f}%")
                    except Exception:
