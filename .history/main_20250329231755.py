...
class MediaManagerApp:
    ...
    async def setup_file_watcher(self) -> FileWatcher:
        ...
        # Use a queue to pass file paths between threads
        file_queue = asyncio.Queue()

        def sync_callback(filepath):
            """
            Synchronous callback to put filepath into queue
            """
            try:
                # Use run_coroutine_threadsafe to safely add to queue from a different  thread
                asyncio.run_coroutine_threadsafe(
                    file_queue.put(filepath), 
                    asyncio.get_event_loop()
                )
            except Exception as e:
                self.logger.error(f"Error queueing file: {e}")

        watcher.create_event_handler(
            on_created=sync_callback,
            extensions=['.mkv', '.mp4', '.avi']
        )

        watcher.start()

        # Add a method to process the queue
        ...
        # Add queue processing to the watcher's run method
        async def enhanced_run():
            queue_task = asyncio.create_task(process_file_queue())
            await watcher.run()
            await queue_task

        return watcher, enhanced_run
