...
class MediaManagerApp:
    ...
    async def _download_media_with_tqdm(self, event, download_path, status_message):
                ...
                try:
                    ...
                    async with download_semaphore, rate_limit_semaphore:
                        ...
                        progress_tracker = ProgressTracker(
                            ...
                            update_callback=lambda progress: 
                                asyncio.create_task(
                                    self._safe_edit_message(
                                        status_message, 
                                        self._create_custom_progress_bar(progress['progress_percent'])
                                    )
                                )
                        )
                        ...
                        await self.telegram_client.download_media(
                            ...
                            progress_callback=lambda current, total: 
                                asyncio.create_task(progress_tracker.update(current - progress_tracker.current_size))
                        )
        ...
        return await rate_limited_download()  # This is now correctly inside an async method
